...
for e in range(epochs): # some amount of generations
    ...
    for i in range(len(networks)): # play through each network
        ...
        for i in range(len(walk_vs_stop[0])):
            ...
            value = agent.forward(input)
            print("__")
            print(ground_truth)
            print("")
            print(value)
            print("")
            print(abs(ground_truth - value))
    ...
    for i in range(len(species)):
        ...
        for j in range(amount):
            if amount == 1 or len(species[i]) == 1: # if only one network in species keep it
                ...
                child = neat_functions.making_children(first_parent, second_parent, observation_space, action_space) # make child and add it
