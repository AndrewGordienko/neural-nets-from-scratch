...
class Agent():
    ...
    def mutate_node(self): # adding node between two connected
        ...
        if abs(random_layer - output_node_layer) == 1: # layer in between needed
            """
            print("layer in between")
            print(f"starting layer {random_layer} node {random_node.number}")
            print(f"middle layer {node.layer_number} node {node.number}")
            print(f"ending layer {output_node_layer+1} node {output_node_number}")
            print("")
            """
            ...
            print(f"starting layer {random_layer} node {random_node.number}")
            print(f"middle layer {node.layer_number} node {node.number}")
            print(f"ending layer {output_node_layer} node {output_node_number}")
            print("")
            """
            self.network[node.layer_number].append(deepcopy(node))
            new_node_index = len(self.network[random_layer + 1]) - 1
